...
- Repair stalled unpublished drafts with `node scripts/gelato-products.mjs --execute --visible --repair-created --repair-batch-photos 1`. The repair deduplicates overlapping Gelato pages, recovers known products by exact ID, polls new products by exact ID, and only clears repair-owned or aged queued products.
