...
- Create or resume the public catalog with `node scripts/gelato-products.mjs --reconcile --execute --content-file /path/to/cms-snapshot.json`. Shopify Admin credentials are required for metadata updates, unarchive, and archive actions. The ignored state file and stable ID/format tags prevent duplicate creation, and API throttling is retried with exponential backoff. Require `node scripts/gelato-products.mjs --strict-audit` to pass before declaring the catalog complete.
