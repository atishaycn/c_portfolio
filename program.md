...
- Repair stalled unpublished drafts with `node scripts/gelato-products.mjs --execute --visible --repair-created --repair-batch-photos 1`. The repair deduplicates overlapping Gelato pages, bounds each API request at five minutes, recovers known products by exact ID, polls new products by exact ID for up to three hours, retries after one hour without progress, and only clears repair-owned or aged queued products.
